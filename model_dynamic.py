...
class MultiStreamTransformerBlock(nn.Module):
    def __init__(self, channels, num_heads, expansion_factor=2.66, res_scale=0.1, streams=2):
        ...
        self.attn_map = nn.Sequential(
            nn.Conv2d(channels * streams, streams, kernel_size=1, bias=False),
            nn.Softmax(dim=1)  # normalize across streams
        )
    ...
    def forward(self, x):
        ...
        for stream in self.streams:
            outs.append(stream(x_norm1))

        # Stack theo channels
        outs_cat = torch.cat(outs, dim=1)  # (B, C*streams, H, W)

        # Predict attention map
        attn_scores = self.attn_map(outs_cat)  # (B, streams, H, W)

        # Weighted sum
        out = 0
        for i in range(len(outs)):
            out = out + outs[i] * attn_scores[:, i:i+1, :, :]

        # Residual Add sau Attention
        x = x + out * self.res_scale
